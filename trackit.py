...
class HexTrack:
    def __init__(self, cfg, nodes, shared_arr, enter):
...
if __name__ == '__main__':
    ...
    print('---------\n  MOUSE Tracker version: v0.1 \n---------\n')

    enter_tm = input('Enter track master    : ')
    enter_mouse_id = input('Enter mouse id    : ')
    enter_date = input('Date of experiment (DD-MM-YY)  : ')
    ...
    if gui.vfile is not None:
        cfg['frame_sources'] = gui.vfile
    ...
    ht = HexTrack(cfg=cfg, nodes=nodes['nodes'], shared_arr=SHARED_ARR, enter = enter)
    ht.loop()
