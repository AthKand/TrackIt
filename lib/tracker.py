...
from util.utilities import extract_metadata, convert_milli
...
class Tracker:
    def __init__(self, cfg, nodes, idx=0, thresh_mask=100, thresh_detect=35, thresh_led=50):
        ...
        self.save_nodes = deque(maxlen=3)
        self.last_node = None
    ...
    def make_mask(self, frame, global_threshold=70):
        ...
        # cv2.imshow(f'mask_{self.id}', self.mask_frame)
        self.has_mask = True
    ...
    def apply(self, frame, record):
        ...
        self.frame = frame                                              #self.frame = frame[f_start:f_end, :]
        ...
        frame_time = int(metadata['ft'][1]) if 'ft' in metadata else None

        #frame_time = tms 
        #print(tms)

        #frame_time = self.img.get(cv2.CAP_PROP_POS_MSEC)
        ...
        if not frame_of_interest:
            ...
            shape_check = not bool(self.img.shape[0] == foi.shape[0] and self.img.shape[1] == foi.shape[1])
            #print(shape_check)
            ...
            cv2.imshow(f'conts_{self.id}', thresh)
            ...
            if largest_cnt is None:
                ...
                self.show_cam = True
                cv2.drawMarker(thresh, (cx, cy), (0, 255, 0), cv2.MARKER_CROSS, 15, 5)
            ...
            if self.last_node != self.active_node and self.active_node is not None:
                ...
                self.save_nodes.appendleft(self.last_node)
                if not record:
            ...
            #Check recorded state and log information
            if record:
                if largest_cnt is not None:
                    converted_time = convert_milli(int(frame_time))
                    if self.active_node:
                        self.log.info(f'Tracker {self.id}: {converted_time} : detected at {(cx, cy)} @ {self.active_node}')
                    else:
                        self.log.info(f'Tracker {self.id}: {converted_time} : detected at: {cx, cy}')
                # else:
                #     self.log.info(f'Tracker {self.id}: Mouse not detected')
