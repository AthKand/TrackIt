...
from util.utilities import buf_to_numpy


class Frame:
    def __init__(self, index, img, source_type, timestamp=None, tickstamp=None, ft=None, add_stamps=True):
        ...
        self.frame_time = ft if ft is not None else time.time()
...
class Grabber(threading.Thread):
    ...
    def run(self):
        ...
        while not self._ev_terminate.is_set():
            ...
            ft = self.capture.get(cv2.CAP_PROP_POS_MSEC)
            ...
            self.frame = Frame(self.n_frames, frame, 'Grabber', ft=ft, add_stamps=self.is_live)
    ...
    def relay_frames(self):
        ...
        with self._shared_arr.get_lock():
            ...
            self.embed_metadata(row=2, label='timest', data=int(self.frame.timestamp))
            self.embed_metadata(row=3, label='ft', data=self.frame.frame_time)
